...
def visualization(request, fid):
    ...
    subplot_titles = ["ego speed = {} m/s".format(2 * i) for i in range(speed_range)]
    ...
    for i in range(len(space_gap)):
        # filter the data that leader speed equals to ego_speed
    ...
    for i in range(len(e_space_gap)):
        if abs(e_accel[i]) < 0.1:  # abs acceleration value < 0.1
    ...
    fig.update_xaxes(
        title_text="speed v m/s", title_font_size=10, range=[0, 30], row=1, col=1
    )
    fig.update_yaxes(
        title_text="gap s/m", title_font_size=10, range=[0, 100], row=1, col=1
    )

    # 2. Phase Space Cross Sections Plot:
    # space gap on the y-axis and lead speed on the x-axis.
    for spd in range(speed_range):
        # filter data
        tmp_space_gap = []
        tmp_lead_speed = []
        tmp_accel = []
        for i in range(len(space_gap)):
            if ego_speed[i] == 2 * spd:  # filter the speed in [0,2,4,...,30]
        ...
        # add vertical dashed line for speed
        fig.add_vline(
            spd * 2,
            row=spd // 4 + 5,
            col=(spd % 4) + 1,
        ...
        # filter the data that speed close to zero
        z_space_gap = []
        z_speed = []
    ...
    chart = fig.to_html()
    return render(request, "visualization.html", context={"chart": chart, "file": file})
